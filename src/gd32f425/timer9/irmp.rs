#[doc = "Register `IRMP` reader"]
pub type R = crate::R<IrmpSpec>;
#[doc = "Register `IRMP` writer"]
pub type W = crate::W<IrmpSpec>;
#[doc = "Internal trigger input1 remap\n\nValue on reset: 0"]
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
#[repr(u8)]
pub enum Iti1Rmp {
    #[doc = "0: TIMER9 internal trigger input 1 is connected to RTC_ALARM output"]
    Rtc = 0,
    #[doc = "1: Reserved"]
    Reserved1 = 1,
    #[doc = "2: Reserved"]
    Reserved2 = 2,
    #[doc = "3: Reserved"]
    Reserved3 = 3,
}
impl From<Iti1Rmp> for u8 {
    #[inline(always)]
    fn from(variant: Iti1Rmp) -> Self {
        variant as _
    }
}
impl crate::FieldSpec for Iti1Rmp {
    type Ux = u8;
}
impl crate::IsEnum for Iti1Rmp {}
#[doc = "Field `ITI1_RMP` reader - Internal trigger input1 remap"]
pub type Iti1RmpR = crate::FieldReader<Iti1Rmp>;
impl Iti1RmpR {
    #[doc = "Get enumerated values variant"]
    #[inline(always)]
    pub const fn variant(&self) -> Iti1Rmp {
        match self.bits {
            0 => Iti1Rmp::Rtc,
            1 => Iti1Rmp::Reserved1,
            2 => Iti1Rmp::Reserved2,
            3 => Iti1Rmp::Reserved3,
            _ => unreachable!(),
        }
    }
    #[doc = "TIMER9 internal trigger input 1 is connected to RTC_ALARM output"]
    #[inline(always)]
    pub fn is_rtc(&self) -> bool {
        *self == Iti1Rmp::Rtc
    }
    #[doc = "Reserved"]
    #[inline(always)]
    pub fn is_reserved1(&self) -> bool {
        *self == Iti1Rmp::Reserved1
    }
    #[doc = "Reserved"]
    #[inline(always)]
    pub fn is_reserved2(&self) -> bool {
        *self == Iti1Rmp::Reserved2
    }
    #[doc = "Reserved"]
    #[inline(always)]
    pub fn is_reserved3(&self) -> bool {
        *self == Iti1Rmp::Reserved3
    }
}
#[doc = "Field `ITI1_RMP` writer - Internal trigger input1 remap"]
pub type Iti1RmpW<'a, REG> = crate::FieldWriter<'a, REG, 2, Iti1Rmp, crate::Safe>;
impl<'a, REG> Iti1RmpW<'a, REG>
where
    REG: crate::Writable + crate::RegisterSpec,
    REG::Ux: From<u8>,
{
    #[doc = "TIMER9 internal trigger input 1 is connected to RTC_ALARM output"]
    #[inline(always)]
    pub fn rtc(self) -> &'a mut crate::W<REG> {
        self.variant(Iti1Rmp::Rtc)
    }
    #[doc = "Reserved"]
    #[inline(always)]
    pub fn reserved1(self) -> &'a mut crate::W<REG> {
        self.variant(Iti1Rmp::Reserved1)
    }
    #[doc = "Reserved"]
    #[inline(always)]
    pub fn reserved2(self) -> &'a mut crate::W<REG> {
        self.variant(Iti1Rmp::Reserved2)
    }
    #[doc = "Reserved"]
    #[inline(always)]
    pub fn reserved3(self) -> &'a mut crate::W<REG> {
        self.variant(Iti1Rmp::Reserved3)
    }
}
impl R {
    #[doc = "Bits 10:11 - Internal trigger input1 remap"]
    #[inline(always)]
    pub fn iti1_rmp(&self) -> Iti1RmpR {
        Iti1RmpR::new(((self.bits >> 10) & 3) as u8)
    }
}
impl W {
    #[doc = "Bits 10:11 - Internal trigger input1 remap"]
    #[inline(always)]
    pub fn iti1_rmp(&mut self) -> Iti1RmpW<'_, IrmpSpec> {
        Iti1RmpW::new(self, 10)
    }
}
#[doc = "channel input remap register\n\nYou can [`read`](crate::Reg::read) this register and get [`irmp::R`](R). You can [`reset`](crate::Reg::reset), [`write`](crate::Reg::write), [`write_with_zero`](crate::Reg::write_with_zero) this register using [`irmp::W`](W). You can also [`modify`](crate::Reg::modify) this register. See [API](https://docs.rs/svd2rust/#read--modify--write-api)."]
pub struct IrmpSpec;
impl crate::RegisterSpec for IrmpSpec {
    type Ux = u32;
}
#[doc = "`read()` method returns [`irmp::R`](R) reader structure"]
impl crate::Readable for IrmpSpec {}
#[doc = "`write(|w| ..)` method takes [`irmp::W`](W) writer structure"]
impl crate::Writable for IrmpSpec {
    type Safety = crate::Unsafe;
}
#[doc = "`reset()` method sets IRMP to value 0"]
impl crate::Resettable for IrmpSpec {}
