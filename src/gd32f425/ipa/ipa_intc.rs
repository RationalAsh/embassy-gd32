...
impl W {
    #[doc = "Bit 0 - Clear bit for transfer access error interrupt flag"]
    #[inline(always)]
    pub fn taeifc(&mut self) -> TaeifcW<'_, IpaIntcSpec> {
        TaeifcW::new(self, 0)
    }
    #[doc = "Bit 1 - Clear bit for full transfer finish interrupt flag"]
    #[inline(always)]
    pub fn tfifc(&mut self) -> TfifcW<'_, IpaIntcSpec> {
        TfifcW::new(self, 1)
    }
    #[doc = "Bit 2 - Clear bit for transfer line mark interrupt flag"]
    #[inline(always)]
    pub fn tlmif(&mut self) -> TlmifW<'_, IpaIntcSpec> {
        TlmifW::new(self, 2)
    }
    #[doc = "Bit 3 - Clear bit for LUT access conflict interrupt flag"]
    #[inline(always)]
    pub fn lacifc(&mut self) -> LacifcW<'_, IpaIntcSpec> {
        LacifcW::new(self, 3)
    }
    #[doc = "Bit 4 - Clear bit for LUT loading finish interrupt flag"]
    #[inline(always)]
    pub fn llfifc(&mut self) -> LlfifcW<'_, IpaIntcSpec> {
        LlfifcW::new(self, 4)
    }
    #[doc = "Bit 5 - Clear bit for wrong configuration interrupt flag"]
    #[inline(always)]
    pub fn cwcfif(&mut self) -> CwcfifW<'_, IpaIntcSpec> {
        CwcfifW::new(self, 5)
    }
}
